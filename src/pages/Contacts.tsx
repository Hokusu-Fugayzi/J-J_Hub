		...
		<div className="border-t border-border p-4">
			...
			{showActivityForm && (
				...
				>
					...
					<button
						...
						className="px-3 py-2 bg-primary text-primary-foreground text-sm rounded-md hover:bg-primary/90 disabled:opacity-50"
					>
						{saving ? "..." : "Log"}
					</button>
				</form>
			...
			<div className="space-y-2">
				{activities.map((act) => (
					<div
						key={act.id}
						className="flex items-start justify-between gap-2 text-xs group/act"
					>
						<div className="flex flex-wrap items-center gap-1.5 min-w-0">
							<span className="px-1.5 py-0.5 rounded bg-muted text-muted-foreground shrink-0">
								{act.type}
							</span>
							<span className="break-words">{act.description}</span>
						...
						<button
							...
							className="md:opacity-0 md:group-hover/act:opacity-100 p-1.5 hover:bg-accent rounded shrink-0"
						>
							<Trash2 className="w-3.5 h-3.5 text-muted-foreground" />
						</button>
					</div>
